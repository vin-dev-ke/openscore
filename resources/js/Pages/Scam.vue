<template>
    ...
    <div class="flex flex-col items-center justify-center mt-8 mb-2">
      ...
      <div class="flex md:flex-nowrap flex-wrap justify-start items-end md:justify-start mb-3 mt-3">
        <div class="relative sm:w-64 w-40 sm:mr-4 mr-2">
          <input v-model="search_term" @keyup="search" name="search" type="text" autocomplete="off" placeholder="Search by email or phone" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:ring-2 focus:bg-transparent focus:ring-indigo-200 focus:border-indigo-500 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div v-for="(scam, index) in scams.data" :key="index">
          <div class="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <!-- Scam Content -->
            <div class="p-4">
              <p class="text-gray-700 leading-relaxed">{{ scam.content }}</p>
            </div>
            <!-- Scam Comments Count -->
    ...
    <!-- Paginator -->
    <div class="order-last mt-4 p-2 flex max-w-7xl ml-80 justify-center items-center py-6 sm:px-6 lg:px-8">
     <Link 
